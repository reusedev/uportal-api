...
func (s *PaymentService) HandleWxPayNotify(ctx context.Context, requestBody []byte, headers map[string]string) error {
	...
	// 获取订单ID
	orderID, err := strconv.ParseInt(*transaction.OutTradeNo, 10, 64)
	if err != nil {
		return fmt.Errorf("parse order id error: %v", err)
	}
	...
	// 检查是否已处理过该通知
	existingRecord, err := model.GetNotifyRecord(tx, orderID, *transaction.TransactionId)
	if err == nil {
		// 如果已处理成功，直接返回成功
		if existingRecord.ProcessStatus == model.NotifyStatusSuccess {
			log.Printf("Notification for order %d, transaction %s already processed successfully", orderID, *transaction.TransactionId)
	...
	// 获取分布式锁
	lockKey := fmt.Sprintf("payment_notify_lock:%d:%s", orderID, *transaction.TransactionId)
	...
	if order.Status == 1 {
		log.Printf("Order %d already paid, skip processing", orderID)
		// 更新通知记录为成功
		now := time.Now()
	...
	log.Printf("Successfully processed payment notification for order %d, transaction %s", orderID, *transaction.TransactionId)
	return nil
}
...
func (s *PaymentService) QueryWxPayOrder(ctx context.Context, orderID int64) (*payments.Transaction, error) {
	...
	if order.PaymentMethod != "wechat" {
		return nil, errors.New(errors.ErrCodeInvalidParams, "不支持的支付方式", nil)
	}
...
func (s *PaymentService) CloseWxPayOrder(ctx context.Context, orderID int64) error {
	...
	// 检查支付方式
	if order.PaymentMethod != "wechat" {
		return errors.New(errors.ErrCodeInvalidParams, "不支持的支付方式", nil)
	}
